...
export async function POST(req: NextRequest) {
  ...
  for (const r of (rates ?? []) as Rate[]) {
    ...
    if (dates.size === 0 && r.monthly_allowance === 0) continue
    ...
    const total = employTotal + contractTotal
    if (total === 0) continue
    results.push({
      user_id: r.user_id, name: name.get(r.user_id) ?? '—',
      employTotal, contractTotal, contractTax, allowance,
      daysCount: days.length, total, note: r.note ?? '', contractorName: r.contractor_name ?? '', days,
    })
  }
  results.sort((a, b) => b.total - a.total)
  const grand = results.reduce((g, r) => ({ employ: g.employ + r.employTotal, contract: g.contract + r.contractTotal, tax: g.tax + r.contractTax, total: g.total + r.total }), { employ: 0, contract: 0, tax: 0, total: 0 })
