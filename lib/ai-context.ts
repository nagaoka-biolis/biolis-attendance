...
function renderDaily(days: DailySales[], doctorByDate: Map<string, DoctorSalesCsv['days'][number]>): string {
  ...
  for (const d of days) {
    ...
    for (const c of d.checkouts) {
      ...
      rows.push(
        `| ${d.date}(${weekday(d.date)}) | ${c.time} | ${who ?? '—'} | ${yen(c.amount)} | ${detail} |`
      )
    }
  }
  ...
  lines.push(
    '患者名とカルテ番号は保存時に削除済みで、ここには含まれない。\n' +
      '担当欄は、会計の金額と医師別売上の金額を突き合わせてこちらで割り出したもの' +
      `（対応がついた会計 ${resolved}件 / つかなかった会計 ${unknown}件）。\n` +
      '**「—」は売上が不明という意味では断じてない。** 医師別売上の金額は' +
      '1円残らずいずれかの医師に割り当てられており、データに欠損はない。' +
      '「—」は、1つの会計の中で施術ごとに担当が違う等の理由で、' +
      '会計と医師の対応が一意に決まらなかっただけである。\n' +
      '「—」の会計の担当を聞かれたら「この会計単位では特定できない」と答えること。' +
      '間違っても「売上が不明」「担当が不明な売上がある」とは言わないこと。'
  )
  lines.push('')
...
## 資料を読むときの注意
- **医師別売上に欠損は無い。** 金額は1円残らずいずれかの医師に割り当てられている。
  「売上が不明」という状態は存在しないので、そう言ってはいけない。
- 「日ごとの施術内容」の担当欄の「—」は、会計と医師の対応がこちらで一意に
  決められなかっただけ。売上が不明という意味ではない。聞かれたら
  「この会計単位では特定できない」と答える。推測はしない。
- 「担当未割当」は ACUSIS 側で担当医が設定されていない会計。入力漏れであって、
  売上が消えているわけではない。7月26,750円→8月158,675円と増えている。
